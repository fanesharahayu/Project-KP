...
- Lokasi: `D:\!!! PENTING JAGAN DIHAPUS !!\.secrets` (file `.secrets` di dalam folder tersebut)
...
- Stack: Node.js + Express + SQLite (`better-sqlite3`), session `express-session`.
- Entry point: `server.js` (port via `process.env.PORT`, default 3000).
- Repo GitHub: `fanesharahayu/Project-KP` (public), branch `main`.
